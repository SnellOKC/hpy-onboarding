var Candidate = require('../models/checklist')
...
exports.candidate_list = function(req, res, next) {
  ...
  Candidate.find()
    .sort([['cand_name', 'ascending']])
    .exec(function (err, list_candidates) {
      if (err) { return next(err); }
      //Successful, so render
      res.render('candidate_list', { title: 'Candidate List', list_candidates:  list_candidates});
    })
...
exports.candidate_create_get = function(req, res) {
    res.render('candidate_form', {title: 'Create New'});
};

// Handle Candidate create on POST
exports.candidate_create_post = function(req, res) {
    req.checkBody('cand_name', 'Candidate name required').notEmpty();

    req.sanitize('cand_name').escape();
    req.sanitize('cand_name').trim();
